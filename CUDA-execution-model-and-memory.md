...
Warps are for **execution scheduling,** Thread blocks are for **program structure, memory sharing and synchronization.** A warp always contains 32 threads. If a thread block contains N threads, the hardware divides them as: number of warps in a block = N/32. 
...
After this step, the memory system still needs to choose **which part of the row** should be sent to the requester. That selection is called a **column access**. Consecutive columns within that row can be read extremely quickly.
...
## H100 (Hopper) Architecture Extensions
...
- 32-bit unit is the base unit for registers. The hardware register file is effectively 32-bit slots. When working with FP16 or FP8 we have to use packed data types to fit 2/4 elements into a single register. 

H100 extends the traditional GPU execution and memory model by introducing mechanisms for explicit latency management, cross-SM cooperation and higher-throughput data movement, enabling more efficient execution of large-scale tensor and distributed workloads.

#### Thread Block Clusters and Distributed Shared Memory

Hopper introduces thread block clusters as a new level in the execution hierarchy that enables coordinated execution and communication across multiple thread blocks spanning different streaming multiprocessors. In pre-Hopper architectures, although multiple thread blocks could reside on the same SM, they remained fully isolated in terms of shared memory and synchronization and any communication between blocks required going through global memory or the L2 cache. Thread block clusters extend this model by grouping a set of thread blocks into a cluster and guaranteeing that all blocks in the cluster are scheduled concurrently on a set of SMs. This guarantee allows the hardware to expose distributed shared memory, which is a unified address space spanning the shared memory regions of all SMs participating in the cluster. Threads in one block can directly read and write data located in the shared memory of another block within the same cluster, with accesses routed through a dedicated on-chip interconnect that is significantly faster than global memory. In addition to shared memory access, clusters introduce cluster-level synchronization primitives that allow thread blocks to coordinate execution using barriers, enabling producer–consumer patterns and other cooperative workflows across SM boundaries.  

Clusters are defined in terms of thread blocks at the programming level, but they implicitly form a cooperative domain of SMs at the hardware level, since those blocks must be co-resident to enable communication and synchronization. This effectively introduces an intermediate communication layer in the memory hierarchy between per-SM shared memory and the global L2 cache. By allowing larger working sets to remain on-chip and enabling direct cross-SM cooperation, thread block clusters significantly improve data locality and coordination for workloads that exceed the capacity of a single SM.

#### Transformer Engine (Dynamic Precision and Scaling)

The Transformer Engine in Hopper is a coordinated system that enables efficient use of low-precision formats, particularly FP8, while preserving numerical stability during training and inference of transformer models. Rather than being a standalone compute unit, it operates in conjunction with Tensor Cores by managing how tensors are represented and scaled throughout computation. Each tensor represented in FP8 is associated with a scaling factor that maps its compact representation to real numerical values. Before a Tensor Core operation, values are effectively scaled so that they fall within the representable range of FP8, and during computation, the Tensor Core interprets these values using their associated scaling. Accumulation is performed in higher precision to avoid loss of accuracy. After computation, results may be rescaled and either kept in higher precision or converted back to FP8 with updated scaling factors. This process is continuous across the computation graph, rather than being tied to specific memory transitions. The Transformer Engine dynamically adjusts scaling based on the distribution of tensor values, allowing aggressive use of FP8 where possible without introducing instability. This reduces memory bandwidth requirements and increases computational throughput while maintaining the correctness of model training and inference.

#### Tensor Memory Accelerator (TMA)

The Tensor Memory Accelerator is a hardware mechanism in Hopper designed to efficiently move structured, multidimensional data between global memory and shared memory. In earlier architectures, memory transfers were performed by individual threads issuing loads, with the hardware attempting to coalesce these accesses into efficient transactions. While flexible, this approach introduced overhead due to per-thread address calculations and instruction issuance. TMA replaces this with a model where a warp can describe a bulk transfer of a tensor region, such as a tile of a matrix, including its shape, strides, and layout. The hardware then performs this transfer using a dedicated engine that optimizes address generation, coalescing, and bandwidth utilization. 

These transfers are asynchronous, meaning that a warp can initiate a data movement operation and continue execution while the transfer proceeds in the background. This enables overlap between data movement and computation, forming a pipeline where one tile is being processed while the next is being loaded. TMA also natively supports strided and higher-dimensional layouts, reducing the need for manual indexing logic and making it well suited for tensor-oriented workloads.

#### Tensor Cores (Hopper Enhancements)

- While Tensor Cores already existed in earlier architectures, Hopper extends them with higher throughput and support for additional numerical formats, most notably FP8.
- Hopper improves the efficiency of feeding data into Tensor Cores by enhancing the surrounding system, including shared memory usage, asynchronous data movement, and better support for structured data layouts.
- Accumulation within Tensor Cores is typically performed in higher precision such as FP16 or FP32, even when inputs are lower precision, to maintain numerical stability.
